...
const WalletAdapters = {
    ...
    "Sollet (Extention)": wallet_adapter_wallets_1.SolletExtensionWalletAdapter,
...
class Adapter {
    constructor(config = {}) {
        ...
        const { network = DEFAULT_NETWORK, wallets = DEFAULT_WALLETS, connectionConfig = { commitment: "confirmed", confirmTransactionInitialTimeout: 100000 }, localStorageKey = DEFAULT_LOCAL_STORAGE_KEY, onConnect, onDisconnect, onError, autoConnect, onConnecting, } = config || {};
    ...
    setupListeners() {
        this.wallets.map((_a) => {
            ...
            if ((adapter === null || adapter === void 0 ? void 0 : adapter.name) === this.localStorageWalletName &&
                (adapter.readyState === wallet_adapter_base_1.WalletReadyState.Loadable ||
    ...
    sendTokenTransaction(reciever, amount, decimals, MINT_PUBLIC_KEY, { onError, onSuccess, onTransactionSent }) {
        var _a;
        return __awaiter(this, void 0, void 0, function* () {
            if (!((_a = this.wallet) === null || _a === void 0 ? void 0 : _a.adapter.publicKey)) {
                onError && onError("Wallet not connected!");
            ...
            const mintPublicKey = new web3_js_1.PublicKey(MINT_PUBLIC_KEY);
            const dp = Math.pow(10, decimals);
            let signature = "";
            try {
                const toWallet = new web3_js_1.PublicKey(reciever);
                let fromTokenAccountData = yield this.connection.getParsedTokenAccountsByOwner(this.wallet.adapter.publicKey, {
                    mint: mintPublicKey
                });
                fromTokenAccountData = fromTokenAccountData.value[0];
                let toTokenAccountsData = yield this.connection.getParsedTokenAccountsByOwner(toWallet, {
                    mint: mintPublicKey
                });
                toTokenAccountsData = toTokenAccountsData.value[0];
                const fromTokenAccount = new web3_js_1.PublicKey(fromTokenAccountData.pubkey);
                const toTokenAccount = new web3_js_1.PublicKey(toTokenAccountsData.pubkey);
                let transaction = new web3_js_1.Transaction().add((0, spl_token_1.createTransferInstruction)(fromTokenAccount, toTokenAccount, this.wallet.adapter.publicKey, amount * dp, [], spl_token_1.TOKEN_PROGRAM_ID));
                signature = yield this.wallet.adapter.sendTransaction(transaction, this.connection);
                onTransactionSent && onTransactionSent(signature);
                yield this.connection.confirmTransaction(signature, "processed");
                onSuccess && onSuccess(signature);
